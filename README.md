...
2. It's analyzed entirely on your device: a visual AI-likelihood classifier
   (when bundled — see [Known accuracy limitations](#known-accuracy-limitations)),
...
- **Visual AI classifier** — on-device ONNX Runtime inference.
...
Drop a verified `.onnx` file at
`app/src/main/assets/models/ai-image-detector.onnx` — nothing else needs to
change; the classifier and Settings screen pick it up automatically.
`tools/convert_model.py` documents exporting the target model
...
- Target classifier model (`Dafilab/ai-image-detector`): Apache-2.0.
- `contentauth/c2pa-android` (referenced, not bundled): dual MIT/Apache-2.0.

## Known accuracy limitations

- **No classifier is bundled by default in this build** — out of the box,
  Genned's likelihood is based on metadata signals only, which is meaningfully
  weaker than with the visual classifier running. The Settings screen states
  this honestly rather than implying full functionality.
- **No independent accuracy benchmark has been run** against the target
  ...
  produce that number once a model and a labeled dataset are available — do
  not treat this app's percentages as validated accuracy until that's been
  done.
